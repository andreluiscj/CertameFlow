...
import { useContratosAtrasadosCount } from '@/hooks/useAtrasadas';
...
const navItems = [
  { to: '/contratos/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
  { to: '/contratos/lista', icon: FileText, label: 'Contratos' },
  { to: '/contratos/acompanhar', icon: Activity, label: 'Acompanhar Contrato' },
  { to: '/contratos/clientes', icon: Users, label: 'Clientes' },
  { to: '/contratos/logs', icon: History, label: 'Logs' },
];

const cadastrosItems = [
  { to: '/contratos/responsaveis', icon: UserCog, label: 'Responsáveis' },
  { to: '/contratos/contas-recebimento', icon: Landmark, label: 'Contas de Recebimento' },
];
...
export function ContratosSidebar() {
  ...
  const atrasadosCount = useContratosAtrasadosCount();
        ...
        >
          ...
          {!isCollapsed && (
            <div className="flex flex-col">
              <h1 className="text-base font-bold text-foreground whitespace-nowrap">Gestão de Contratos</h1>
            ...
            const linkContent = (
              ...
              >
                ...
                >
                  ...
                  />
                  {item.to === '/contratos/acompanhar' && atrasadosCount > 0 && (
                ...
                {!isCollapsed && (
                  <>
                    ...
                    {item.to === '/contratos/acompanhar' && atrasadosCount > 0 && (
                      <span
                        className="ml-auto rounded-md bg-destructive px-2 py-0.5 text-[10px] font-bold text-destructive-foreground whitespace-nowrap"
                        title={`${atrasadosCount} ${atrasadosCount === 1 ? 'contrato com parcela atrasada' : 'contratos com parcelas atrasadas'}`}
                      >
                        {atrasadosCount} {atrasadosCount === 1 ? 'Atrasado' : 'Atrasados'}
                      </span>
                    )}
                    {isActive && (
